...
int botan_ec_scalar_equal(botan_ec_scalar_t x_w, botan_ec_scalar_t y_w) {
   return BOTAN_FFI_VISIT(x_w, [=](const auto& x) -> int { return x.is_eq(safe_get(y_w)) ? 1 : 0; });
}
...
int botan_ec_scalar_g_mul(botan_ec_scalar_t ec_scalar, botan_rng_t rng, botan_ec_point_t* res) {
   if(Botan::any_null_pointers(res)) {
      return BOTAN_FFI_ERROR_NULL_POINTER;
   }
   return ffi_guard_thunk(__func__, [=]() -> int {
      Botan::EC_AffinePoint pt = Botan::EC_AffinePoint::g_mul(safe_get(ec_scalar), safe_get(rng));
...
int botan_ec_point_mul(botan_ec_point_t* result,
   ...
   return BOTAN_FFI_VISIT(ec_point, [=](auto& pt) -> int {
      ...
      return ffi_new_object(result, std::make_unique<Botan::EC_AffinePoint>(res));
   });
}
