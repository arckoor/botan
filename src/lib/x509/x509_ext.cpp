...
void IPAddressBlocks::IPAddressFamily::decode_from(Botan::BER_Decoder& from) {
   const BER_Object obj = from.get_next_object();
   const ASN1_Type type_tag = obj.type_tag();
...
IPAddressBlocks::IPAddressFamily IPAddressBlocks::merge(std::vector<IPAddressFamily>& blocks) {
   ...
   if(blocks.empty()) {
      throw Exception("at least 1 block is needed to merge");
   }
...
template <typename T>
std::optional<std::vector<T>> sort_and_merge_ranges(std::optional<std::vector<T>> ranges) {
   ...
   std::vector<T>& current = ranges.value();
   if(current.empty()) {
      return ranges;
   }
   bool changed = true;
...
template <IPAddressBlocks::Version V>
IPAddressBlocks::IPAddressChoice<V>::IPAddressChoice(const std::vector<IPAddressBlocks::IPAddressOrRange<V>>& ranges) {
...
void ASBlocks::ASIdentifiers::decode_from(Botan::BER_Decoder& from) {
   ...
   const BER_Object elem_obj = obj_ber.get_next_object();
   ...
   if(elem_type_tag == 0) {
      ...
      const BER_Object rdi_obj = obj_ber.get_next_object();
   ...
   if(elem_type_tag == 1) {
      ...
      rdi_obj_ber.decode(rdi);
      m_rdi = rdi;
      const BER_Object end = obj_ber.get_next_object();
