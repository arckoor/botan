...
void IPAddressBlocks::IPAddressFamily::encode_into(Botan::DER_Encoder& into) const {
   ...
   if(std::holds_alternative<IPAddressChoice<IPv4>>(this->m_ip_addr_choice)) {
      auto ipv4_choice = std::get<IPAddressChoice<IPv4>>(this->m_ip_addr_choice);
      into.encode(ipv4_choice);
   } else {
      auto ipv6_choice = std::get<IPAddressChoice<IPv6>>(this->m_ip_addr_choice);
...
void IPAddressBlocks::IPAddressFamily::decode_from(Botan::BER_Decoder& from) {
   ...
   std::size_t addr_family_length = addr_family.size();
...
void IPAddressBlocks::IPAddressOrRange<V>::encode_into(Botan::DER_Encoder& into) const {
   std::array<uint8_t, V> min = this->m_min.value();
   std::array<uint8_t, V> max = this->m_max.value();
   ...
   for(int i = static_cast<uint8_t>(V - 1); i >= 0; i--) {
      if(!zeros_done) {
         uint8_t local_zeros = std::countr_zero(min[i]);
         zeros += local_zeros;
         zeros_done = (local_zeros != 8);
      }

      if(!ones_done) {
         uint8_t local_ones = std::countr_one(max[i]);
         ones += local_ones;
   ...
   for(size_t i = 0; i < static_cast<uint8_t>(V - discarded_octets - 1); i++) {
      if(min[i] != max[i]) {
         octets_match = false;
         break;
      }
   ...
   uint8_t shifted_min = (min[V - discarded_octets - 1] >> unused_bits);
   uint8_t shifted_max = (max[V - discarded_octets - 1] >> unused_bits);
   bool used_bits_match = (shifted_min == shifted_max);
   ...
   if(octets_match && used_bits_match) {
      ...
      for(size_t i = 0; i < static_cast<uint8_t>(V - discarded_octets); i++) {
      ...
      // compress the min/max address by setting unused bits to 0
      max[V - discarded_octets_max - 1] >>= unused_bits_max;
      max[V - discarded_octets_max - 1] <<= unused_bits_max;
      ...
      for(size_t i = 0; i < static_cast<uint8_t>(V - discarded_octets_min); i++) {
         compressed_min.push_back(min[i]);
      }

      compressed_max.push_back(unused_bits_max);
      for(size_t i = 0; i < static_cast<uint8_t>(V - discarded_octets_max); i++) {
...
void IPAddressBlocks::IPAddressOrRange<V>::decode_from(Botan::BER_Decoder& from) {
   ...
   if(type_tag == ASN1_Type::BitString) {
      ...
      for(int i = V; i >= V - discarded_octets; i--) {
         prefix[i] = 0xff;
      }

      // set all unused bits to 1
      for(int i = 0; i < unused; i++) {
         prefix[V - discarded_octets - 1] |= (1 << i);
      }
      ...
      for(size_t i = 0; i < V - (addr_min_obj.length() - 1); i++) {
         address.push_back(0);
      }
      ...
      for(size_t i = 0; i < V - (addr_max_obj.length() - 1); i++) {
         address.push_back(255);
      }

      for(int i = 0; i < unused_max; i++) {
         address[addr_max_obj.length() - 2] |= (1 << i);
      }
