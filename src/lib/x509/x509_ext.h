   ...
   public:
      ...
      class IPAddress {
         public:
            ...
            const std::array<uint8_t, V> value() const { return m_value; }

         private:
            std::array<uint8_t, V> m_value;
      };
      ...
      class IPAddressFamily : public ASN1_Object {
         public:
            ...
            const std::variant<IPAddressChoice<IPv4>, IPAddressChoice<IPv6>>& addr_choice() const {
               return m_ip_addr_choice;
            }

            void set_addr_choice(const std::variant<IPAddressChoice<IPv4>, IPAddressChoice<IPv6>>& choice) {
               m_ip_addr_choice = choice;
            }

         private:
            std::vector<uint8_t> m_addr_family;
            std::variant<IPAddressChoice<IPv4>, IPAddressChoice<IPv6>> m_ip_addr_choice;
      };
